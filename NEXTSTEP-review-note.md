...
하나의 메서드라도 여러가지 시나리오가 발생할 수 있는데요. 예를들어 if문으로 2가지 시나리오가 발생한다던가, 다양한 인자값에 대응해야 하는 경우의 시나리오도 모두 테스트 할 수 있죠.

이렇게 봤을때 지금은 테스트 커버리지가 좀 낮은거 같아요.. 이부분에 대해서도 고민해보셨으면 좋겠어요. view 객체는 테스트 하지 않으셔도 되지만 지금 view에는 중요한 로직이 들어있기때문에 테스트가 필요해보이네요. 따로 코멘트 드린 것처럼 별도의 객체로 분리한 다음에 해당 객체를 테스트 하는것이 좋겠습니다.
