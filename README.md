...
## (21.10.26) 모든 의존 방향은 도메인을 향해야 한다.
- 의존한다는 것은 의존 하는 것에 영향을 받겠다는 소리다. 도메인이 외부 API, UI, Persistence 시스템에 변화에 반응해서는 안된다. 고로, 모든 의존 방향은 도메인을 향해야 한다.
- 이전에는 도메인이라 함은 == Entity라고 생각했었는데, 도메인은 엔티티가 아니다. 핵심이 되는 Use case의 로직이다.
